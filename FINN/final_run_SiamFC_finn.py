...
from FINN.final_SiamNet_finn import SiamNet, NetType
...
def run_tracker(p, net_type, net_base_path, model_name):
    ...
    np.save("./input_prepare/z_crop_tensor.npy", z_crop_tensor)
    z_features = net.conv_features(Variable(z_crop_tensor).to(device))
    np.save("./input_prepare/z_output.npy", z_features.cpu().detach().numpy())
    ...
    for i in tqdm(range(0, len(img_list))):
        if i > 0:
            ...
            # get features of search regions
            np.save("./input_prepare/x_crops_tensor.npy", x_crops_tensor)

            x_features = net.conv_features(Variable(x_crops_tensor).to(device))
            np.save("./input_prepare/x_output.npy", x_features.cpu().detach().numpy())
            exit()
...
def get_specific_video_results(net_type, net_base_path, model_name, video_name, vis=False, save_to_file=False, p=Config()):
    # get the default parameters
    p.visualization = vis
...
if __name__ == "__main__":
    ...
    p.examplar_size = 130
    p.score_size = 22
    # get_results(NetType.FINN_W2_A2_X28, "models_FINN_W2_A2_28_2020-11-20 20:51:13.788645",
    #             "SiamFC_30_model.pth", p=p)

    # get_results(NetType.FINN_W2_A2_X29, "models_FINN_W2_A2_29_2020-11-20 21:42:57.229287",
    #             "SiamFC_30_model.pth", p=p)

    # get_results(NetType.FINN_W2_A2_X30, "models_FINN_W2_A2_30_2020-11-20 22:10:22.558939",
    #             "SiamFC_30_model.pth", p=p)
    #
    # get_results(NetType.FINN_W2_A2_X31, "models_FINN_W2_A2_31_2020-11-20 22:31:15.789335",
    #             "SiamFC_30_model.pth", p=p)

    get_specific_video_results(NetType.FINN_W2_A2_X29, "models_FINN_W2_A2_29_2020-11-20 21:42:57.229287",
                "SiamFC_30_model.pth", "tc_Thunder_ce", p=p)
